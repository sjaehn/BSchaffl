...
template<size_t sz> bool Shape<sz>::validateNode (const size_t nr)
{
	...
	if (nr >= nodes_.size)
	{
		fprintf (stderr, "BSchaffl.lv2: Node validation called with invalid parameters (node: %li).\n", nr);
		return false;
	}
	...
	{
		fprintf (stderr, "BSchaffl.lv2: Corrupt node data at node %li (%f, %f). Reset shape.\n", nr, nodes_[nr].point.x, nodes_[nr].point.y);
		setDefaultShape ();
		return false;
	}
